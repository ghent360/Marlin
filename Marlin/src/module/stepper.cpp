...
uint32_t inline NS_PER_PULSE_TIMER_TICK() {
  return 1000000000UL / (STEPPER_TIMER_RATE);
}

// Round up when converting from ns to timer ticks
uint32_t inline NS_TO_PULSE_TIMER_TICKS(uint32_t NS) {
  uint32_t ntPerPulseTimerTick = NS_PER_PULSE_TIMER_TICK();
  return (NS + ntPerPulseTimerTick / 2) / ntPerPulseTimerTick;
}
