...
static inline uint8_t swSpiTransfer_mode_0(uint8_t b, const uint8_t spi_speed, const pin_t miso_pin=-1) {
  for (uint8_t i = 0; i < 8; i++) {
    if (spi_speed == 0) {
      digitalWrite(DOGLCD_MOSI, !!(b & 0x80));
      digitalWrite(DOGLCD_SCK, HIGH);
      b <<= 1;
      if (miso_pin >= 0 && digitalRead(miso_pin)) b |= 1;
      digitalWrite(DOGLCD_SCK, LOW);
    }
    else {
      const uint8_t state = (b & 0x80) ? HIGH : LOW;
      for (uint8_t j = 0; j < spi_speed; j++)
        digitalWrite(DOGLCD_MOSI, state);
...
static inline uint8_t swSpiTransfer_mode_3(uint8_t b, const uint8_t spi_speed, const pin_t miso_pin=-1) {
  for (uint8_t i = 0; i < 8; i++) {
    ...
    if (spi_speed == 0) {
      digitalWrite(DOGLCD_SCK, LOW);
      digitalWrite(DOGLCD_MOSI, state);
      digitalWrite(DOGLCD_MOSI, state);  // need some setup time
      digitalWrite(DOGLCD_SCK, HIGH);
    }
    else {
      for (uint8_t j = 0; j < spi_speed + (miso_pin >= 0 ? 0 : 1); j++)
...
static uint8_t swSpiInit(const uint8_t spi_speed) {
  ...
  #endif
  pinMode(DOGLCD_A0, OUTPUT);
  pinMode(DOGLCD_SCK, OUTPUT);
  pinMode(DOGLCD_SCK, OUTPUT);
  pinMode(DOGLCD_SCK, OUTPUT);
  digitalWrite(DOGLCD_SCK, LOW);
  digitalWrite(DOGLCD_MOSI, LOW);
...
uint8_t u8g_com_HAL_stm32f4_sw_spi_fn(u8g_t *u8g, uint8_t msg, uint8_t arg_val, void *arg_ptr) {
  switch (msg) {
    ...
    case U8G_COM_MSG_RESET:
      #if PIN_EXISTS(LCD_RESET)
        digitalWrite(LCD_RESET_PIN, arg_val);
      #endif
      break;
    ...
    case U8G_COM_MSG_CHIP_SELECT:
        ...
        if (arg_val) {               // SCK idle state needs to be set to the proper idle state before
          ...
          digitalWrite(DOGLCD_SCK, HIGH);   // Set SCK to mode 3 idle state before CS goes active
          digitalWrite(DOGLCD_CS, LOW);
        }
        else {
          digitalWrite(DOGLCD_CS, HIGH);
          digitalWrite(DOGLCD_SCK, LOW);  // Set SCK to mode 0 idle state after CS goes inactive
        }
      #else
        digitalWrite(DOGLCD_CS, !arg_val);
      #endif
      break;
    ...
    case U8G_COM_MSG_ADDRESS: /* define cmd (arg_val = 0) or data mode (arg_val = 1) */
      digitalWrite(DOGLCD_A0, arg_val);
      break;
  }
  return 1;
}
